"""
email_service.py — Email Service Sewain
Mendukung 2 backend: Resend HTTP API (default) atau SMTP (fallback).
"""
...
# Provider: "resend" (HTTP API, recommended) atau "smtp" (Sumopod/Gmail/dll)
...
def create_email_token(user_id: int, purpose: str, expires_hours: int) -> str:
    """Buat JWT token untuk email verification atau reset password."""
...
def decode_email_token(token: str, expected_purpose: str) -> dict | None:
    """
    Decode dan validasi email token.
...
def _send_via_resend(to_email: str, subject: str, html_body: str) -> bool:
    """Kirim email via Resend HTTP API."""
...
def _send_via_smtp(to_email: str, subject: str, html_body: str) -> bool:
    """Kirim email via SMTP SSL (port 465)."""
...
def _send_email(to_email: str, subject: str, html_body: str) -> bool:
    """
    Kirim email via provider yang dipilih (resend / smtp).
